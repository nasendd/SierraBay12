		...
		switch(state)
			if(AIRLOCK_CLOSED)
				if(lights && locked)
					...
					new_overlays += overlay_image(bolts_file, plane = LIGHTING_LAMPS_PLANE)
			...
			if(AIRLOCK_DENY)
				if(lights)
					...
					new_overlays += overlay_image(deny_file, plane = LIGHTING_LAMPS_PLANE)
			...
			if(AIRLOCK_EMAG)
				...
				new_overlays += overlay_image(emag_file, plane = LIGHTING_LAMPS_PLANE)
			if(AIRLOCK_CLOSING)
				if(lights)
					new_overlays += overlay_image(lights_file, plane = EFFECTS_ABOVE_LIGHTING_PLANE, layer = ABOVE_LIGHTING_LAYER)
					new_overlays += overlay_image(lights_file, plane = LIGHTING_LAMPS_PLANE)

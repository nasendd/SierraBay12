/obj/item/bouquet
	...
	icon = 'mods/bouquet_by_hakso/icons/bouquet.dmi'
...
/obj/item/bouquet/on_update_icon()
	for(var/obj/item/reagent_containers/food/snacks/grown/flower as anything in contentflovers)
		switch (flower.plantname)
			if ("harebells")
				AddOverlays("flower_1")
			if ("poppies")
				AddOverlays("flower_2")
			if ("lavender")
				AddOverlays("flower_3")
			if ("sunflowers")
				AddOverlays("flower_4")
	. = ..()

/obj/item/bouquet/use_tool(obj/item/weapon, mob/user)
...
/obj/item/reagent_containers/food/drinks/glass2/carafe/use_tool(obj/item/I, mob/user)

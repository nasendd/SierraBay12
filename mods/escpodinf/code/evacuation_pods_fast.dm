...
/datum/evacuation_controller/starship/fast/launch_evacuation()
	if(emergency_evacuation)
		SSwebhooks.send(WEBHOOK_ROUNDEND, list("reason" = "Получен аварийный сигнал с борта **[GLOB.using_map.full_name]**! Это эвакуация?"))
	else
		SSwebhooks.send(WEBHOOK_ROUNDEND, list("reason" = "Потерян контакт с **[GLOB.using_map.full_name]**. К счастью, это всего лишь на время БлюСпейс прыжка."))
	. = ..()
...
/datum/evacuation_controller/starship/fast/call_evacuation(mob/user, _emergency_evac, forced, skip_announce, autotransfer)
...
/datum/evacuation_controller/starship/fast/handle_evac_option(option_target, mob/user, force)
...
/datum/evacuation_option/abandon_ship/fast/execute(mob/user, force_evac = FALSE)
	if (!evacuation_controller)
		return

...
/client/verb/link_discord_account()
	...
	if(dbcon.IsConnected())
		...
		to_chat(usr, SPAN_CLASS("darkmblue", "Для завершения используйте команду: <br> [SPAN_CLASS("boldannounce", "/verify server:Sierra code:[token]")] <br> в канале <b>#bot-chat</b> в Discord-сообществе! <br> Если бот в Discord сообщил, что аккаунт успешно привязан, то требуется переподключиться к серверу (File => Reconnect). <br> Celadon Discord: https://discord.com/invite/rxsggTJzY3"))
		load_player_discord(usr)

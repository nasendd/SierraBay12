...
/datum/computer_file/program/codeprocessor/proc/open_file(filename)
...
/datum/computer_file/program/codeprocessor/proc/save_file(filename)
	var/datum/computer_file/data/F = computer.get_file(filename)
	if(!F) //try to make one if it doesn't exist
		F = computer.create_file(filename, loaded_data, /datum/computer_file/data/text)
		return !isnull(F)
	...
	F.calculate_size()
	if(!HDD.save_data_file(F))
		HDD.save_data_file(backup)
		return 0
	is_edited = 0
	return 1
...
/datum/computer_file/program/codeprocessor/Topic(href, href_list)
	...
	if(href_list["PRG_newfile"])
		...
		switch(newfiletype)
			if("BAT")
				F = computer.create_data_file(newname, "", /datum/computer_file/data/coding/batch)
				var/datum/computer_file/data/coding/batch/FT = F
				code_lang = FT.code_lang
				code_lang_desc = FT.code_lang_desc
			if("TXT")
				F = computer.create_data_file(newname, "", /datum/computer_file/data/text)
			if("CFG")
				F = computer.create_data_file(newname, "", /datum/computer_file/data/config)
			if("DAT")
				F = computer.create_data_file(newname, "", /datum/computer_file/data)
	...
	if(href_list["PRG_saveasfile"])
		...
		if(!newname)
			return 1
		var/datum/computer_file/data/F = computer.create_data_file(newname, loaded_data, /datum/computer_file/data/text)
		if(F)
			open_file = F.filename
		else
			error = "I/O error: Unable to create file '[href_list["PRG_saveasfile"]]'."
...
/datum/nano_module/program/computer_codeprocessor/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1, datum/topic_state/state = GLOB.default_state)
	...
	if (!ui)
		ui = new(user, src, ui_key, "mods-code_processor.tmpl", "DAIS Text 3", 575, 700, state = state)
